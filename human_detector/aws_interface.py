...
def process_video(video_id, video_analyzer,
    ...
    video_doc["process_steps"].append({"step": "human_detection", "state": "running"})
    ...
    try:
        ...
        result_key = os.path.join(os.path.dirname(video_key_path), 'human_detection', video_name + '.json')
        ...
        video_doc["process_steps"][-1]["state"] = "done"
        video_doc["process_steps"][-1]["result_file"] = {"bucket": video_s3_bucket, "key": result_key}
        send_video_info_to_dynamo_db(dyndb_region_id, dyndb_tableId, video_doc)

    except Exception as e:
        # update dynamoDB document
        video_doc["process_steps"][-1]["state"] = "error"
        send_video_info_to_dynamo_db(dyndb_region_id, dyndb_tableId, video_doc)
        raise e
...
if __name__ == "__main__":
    ...
    sqs_queue_name = 'derby-call-humandetector'
